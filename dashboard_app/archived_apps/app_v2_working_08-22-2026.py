...
st.caption(
    ...
    "data pipelines, trend analysis, moving-average calculations, "
...
if not ma_table.empty:
    ...
    st.warning(
        "No moving-average data is available for this asset."
    )
...
# Candidate column naming patterns
ma_column_candidates = {
    "EMA20": [
        f"{selected_ticker}_EMA20"
    ],
    "MA30": [
        f"{selected_ticker}_30"
    ],
    "MA50": [
        f"{selected_ticker}_50"
    ],
    "MA100": [
        f"{selected_ticker}_100"
    ],
    "MA200": [
        f"{selected_ticker}_200"
    ],
}
for label, candidates in ma_column_candidates.items():

    found_col = None

    for candidate in candidates:

        if candidate in ma_wide.columns:
            found_col = candidate
            break
...
1. Market price data is acquired through the Python data pipeline.
